...
pub(crate) async fn new_game() -> Html<String> {
    ...
    let board = board.into_ergo_board();

    let board_html = Rank::all_ranks_ascending().map(|rank| {
...
fn create_option_piece_svg(piece: Option<Piece>) -> Html<String> {
    let piece_svg = if let Some(piece) = piece {
        get_piece_svg(piece)
    } else {
        Html(String::with_capacity(0))
    };

    return piece_svg;
}

fn get_piece_svg(piece: Piece) -> Html<String>{
    let html = match piece.player() {
        Player::White => {
            match piece.kind() {
                PieceKind::Pawn => {
                    html!{
